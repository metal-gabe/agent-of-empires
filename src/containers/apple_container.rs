...
impl ContainerRuntimeInterface for AppleContainer {
   ...
   fn is_container_running(
      ...
      let out_json: Value =
         serde_json::from_slice(&output.stdout).map_err(|e| DockerError::CommandFailed(e.to_string()))?;
   ...
   fn exec_command(
      ...
      if let Some(opt_str) = options {
         ["container", "exec", "-it", opt_str, name, "sh", "-c", &cmd_str].join(" ")
      } else {
...
mod tests {
   ...
   fn test_apple_container_image_exists_locally_nonexistent() {
      if let Some(apple_container) = get_apple_container_runtime_if_available() {
         assert!(!apple_container.image_exists_locally("nonexistent-image-that-does-not-exist:v999"));
      }
   }
