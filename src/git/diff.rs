...
pub fn check_merge_base_status(
   repo_path: &Path,
   base_branch: &str,
) -> Option<String> {
...
pub fn compute_file_diff(
   ...
   let old_is_binary = old_bytes.as_ref().map(|b| is_binary_bytes(b)).unwrap_or(false);
   ...
   let new_is_binary = new_bytes.as_ref().map(|b| is_binary_bytes(b)).unwrap_or(false);

   let is_binary = old_is_binary || new_is_binary;

   // Convert to strings (safe now that we've checked for binary)
   let old_content = old_bytes.and_then(|b| String::from_utf8(b).ok()).unwrap_or_default();
   let new_content = new_bytes.and_then(|b| String::from_utf8(b).ok()).unwrap_or_default();
...
pub fn get_working_file_content(
   repo_path: &Path,
   file_path: &Path,
) -> Result<String> {
   let repo = super::open_repo_at(repo_path)?;
...
pub fn save_working_file_content(
   repo_path: &Path,
   file_path: &Path,
   content: &str,
) -> Result<()> {
...
mod tests {
   ...
   fn test_merge_base_in_worktree() {
      ...
      {
         let first_commit = repo.revparse_single("main~1").unwrap().peel_to_commit().unwrap();
   ...
   fn test_check_merge_base_status_warns_on_unrelated_histories() {
      ...
      let sig = git2::Signature::now("Test", "test@example.com").unwrap();
      {
         ...
         let oid = repo.commit(None, &sig, &sig, "Orphan commit", &tree, &[]).unwrap();
