...
pub(crate) fn open_repo_at(path: &Path) -> std::result::Result<git2::Repository, git2::Error> {
   git2::Repository::open_ext(path, git2::RepositoryOpenFlags::NO_SEARCH, std::iter::empty::<&OsStr>())
}
...
impl GitWorktree {
   ...
   pub fn is_git_repo(path: &Path) -> bool {
      open_repo_at(path).is_ok() || Self::find_main_repo_from_linked_worktree_gitfile(path).is_some()
   }
   ...
   pub fn is_bare_repo(path: &Path) -> bool {
      open_repo_at(path).map(|repo| repo.is_bare()).unwrap_or(false)
   }
   ...
   fn find_main_repo_from_worktree_gitdir(gitdir: &Path) -> Option<PathBuf> {
      ...
      if git_or_bare_dir.file_name() == Some(OsStr::new(".git")) || parent_dir.join(".git").exists() {
      ...
      if create_branch {
         ...
         let commit_oid = if let Ok(head) = repo.head() {
            ...
            repo
               ...
               .ok_or_else(|| GitError::WorktreeCommandFailed("No commits found to branch from".to_string()))?
         };
               ...
               .map(|branches| {
                  branches.filter_map(|b| b.ok()).any(|(b, _)| {
                     b.name()
                        .ok()
                        .flatten()
                        .map(|name| name.ends_with(&format!("/{}", branch)) || name == branch)
                        .unwrap_or(false)
                  })
               })
               .unwrap_or(false);
...
mod tests {
   ...
   fn test_remove_worktree_deletes_worktree() {
      ...
      let git_wt = GitWorktree::new(repo_path.to_path_buf()).unwrap();
      git_wt.create_worktree("removable", &wt_path, false).unwrap();
   ...
   fn test_compute_path_with_template() {
      ...
      let path = git_wt.compute_path("feat/test", template, "abc123").unwrap();
   ...
   fn setup_sibling_bare_repo_worktree() -> Option<(TempDir, PathBuf, PathBuf)> {
      ...
      let clone = std::process::Command::new("git")
         .args(["clone", bare_repo_path.to_str().unwrap(), seed_path.to_str().unwrap()])
         .output()
         .ok()?;
   ...
   fn test_find_main_repo_from_direct_bare_repo_path_returns_bare_repo_path() {
      let Some((_dir, bare_repo_path, _worktree_path)) = setup_sibling_bare_repo_worktree() else {
         return;
      };
   ...
   fn test_delete_branch_deletes_local_branch() {
      ...
      assert!(repo.find_branch("to-delete", git2::BranchType::Local).is_ok());
   ...
   fn test_create_worktree_from_remote_branch() {
      ...
      remote_repo.branch("remote-only-branch", &commit, false).unwrap();
      ...
      git_wt.create_worktree("remote-only-branch", &wt_path, false).unwrap();
   ...
   fn test_create_worktree_succeeds_after_stale_directory_deleted() {
      ...
      let git_wt = GitWorktree::new(repo_path.to_path_buf()).unwrap();
      git_wt.create_worktree("stale-branch", &wt_path, false).unwrap();
      assert!(wt_path.exists());
   ...
   fn test_create_worktree_returns_error_on_git_failure() {
      ...
      let git_wt = GitWorktree::new(repo_path.to_path_buf()).unwrap();
      git_wt.create_worktree("fail-branch", &wt_path, false).unwrap();
   ...
   fn test_regular_repo_create_worktree_existing_branch() {
      ...
      git_wt.create_worktree("feature-a", &wt_path, false).unwrap();
   ...
   fn test_regular_repo_full_builder_flow() {
      ...
      let wt_path = git_wt.compute_path("my-branch", template, "abc12345").unwrap();
   ...
   fn test_linked_bare_repo_create_worktree_new_branch() {
      ...
      git_wt.create_worktree("new-feature", &wt_path, true).unwrap();

      assert!(wt_path.exists(), "Worktree directory should be created");
      assert!(wt_path.join(".git").exists(), "Worktree should have .git file");
   }
   ...
   fn test_linked_bare_repo_create_worktree_existing_branch() {
      ...
      git_wt.create_worktree("existing-branch", &wt_path, false).unwrap();
   ...
   fn test_linked_bare_repo_create_worktree_from_worktree_dir() {
      ...
      git_wt.create_worktree("from-wt-branch", &wt_path, true).unwrap();
   ...
   fn test_linked_bare_repo_full_builder_flow() {
      ...
      let wt_path = git_wt.compute_path("builder-branch", template, "abc12345").unwrap();

      git_wt.create_worktree("builder-branch", &wt_path, true).unwrap();

      assert!(wt_path.exists(), "Worktree should be created at computed path");
      assert!(wt_path.join(".git").exists());
   ...
   fn test_linked_bare_repo_full_builder_flow_from_root() {
      ...
      let template = "./{branch}";
      let wt_path = git_wt.compute_path("root-branch", template, "xyz99999").unwrap();

      git_wt.create_worktree("root-branch", &wt_path, true).unwrap();
   ...
   fn test_sibling_bare_repo_create_worktree_new_branch() {
      ...
      git_wt.create_worktree("new-sibling", &wt_path, true).unwrap();
   ...
   fn test_sibling_bare_repo_create_worktree_existing_branch() {
         ...
         let oid = repo
            ...
            .or_else(|| {
               repo
                  .branches(Some(git2::BranchType::Local))
                  .ok()
                  .and_then(|mut branches| branches.find_map(|b| b.ok().and_then(|(b, _)| b.get().target())))
            })
      ...
      git_wt.create_worktree("existing-sibling", &wt_path, false).unwrap();
   ...
   fn test_sibling_bare_repo_create_worktree_from_worktree_dir() {
      let Some((_dir, _bare_repo_path, worktree_path)) = setup_sibling_bare_repo_worktree() else {
         return;
      };
      ...
      git_wt.create_worktree("from-wt-sibling", &wt_path, true).unwrap();
