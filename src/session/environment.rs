...
pub(crate) const DEFAULT_TERMINAL_ENV_VARS: &[&str] = &["TERM", "COLORTERM", "FORCE_COLOR", "NO_COLOR"];
...
pub(crate) fn resolve_env_value(val: &str) -> Option<String> {
   if let Some(rest) = val.strip_prefix("$$") {
      ...
      match std::env::var(var_name) {
         Ok(v) => Some(v),
         Err(_) => {
            tracing::warn!("Environment variable ${} is not set on host, skipping", var_name);
            None
         },
      }
   } else {
...
pub(crate) fn collect_environment(
   ...
   let entries: &[String] = sandbox_info.extra_env.as_deref().unwrap_or(&sandbox_config.environment);
      ...
      if let Some((key, value)) = entry.split_once('=') {
         ...
         if seen_keys.insert(entry.clone()) {
            match std::env::var(entry) {
               ...
               Err(_) => {
                  tracing::warn!("Environment variable {} is not set on host, skipping", entry);
               },
            }
         }
      }
...
pub(crate) fn build_docker_env_args(sandbox: &SandboxInfo) -> String {
   ...
   tracing::debug!("build_docker_env_args: resolved {} env pairs", env_pairs.len());
...
mod tests {
   ...
   fn test_collect_environment_passthrough() {
      ...
      assert!(result.iter().any(|(k, v)| k == "AOE_TEST_ENV_PT" && v == "test_value"));
   ...
   fn test_collect_environment_extra_env() {
      ...
      let result = collect_environment(&config, &info);
      assert!(result.iter().any(|(k, v)| k == "AOE_TEST_EXTRA" && v == "extra_val"));
   ...
   fn test_collect_environment_falls_back_to_config_when_no_extra() {
      ...
      let result = collect_environment(&config, &info);
      assert!(result.iter().any(|(k, v)| k == "CONFIG_KEY" && v == "config_val"));
   }
   ...
   fn test_collect_environment_dollar_ref() {
      ...
      let result = collect_environment(&config, &info);
      assert!(result.iter().any(|(k, v)| k == "INJECTED" && v == "host_val"));
   ...
   fn test_collect_environment_dollar_dollar_escape() {
      ...
      let result = collect_environment(&config, &info);
      assert!(result.iter().any(|(k, v)| k == "ESCAPED" && v == "$LITERAL"));
   }
   ...
   fn test_build_docker_env_args_with_extra_env() {
      ...
      assert!(result.contains("MY_TOKEN"), "Expected MY_TOKEN in args: {}", result);
      assert!(result.contains("secret123"), "Expected secret123 in args: {}", result);
   ...
   fn test_build_docker_env_args_bare_key() {
      ...
      assert!(result.contains("barevalue"), "Expected barevalue in args: {}", result);
