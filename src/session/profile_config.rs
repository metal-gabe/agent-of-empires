...
use super::config::{Config, ContainerRuntimeName, DefaultTerminalMode, TmuxMouseMode, TmuxStatusBarMode};
...
pub fn apply_tmux_overrides(
   target: &mut super::config::TmuxConfig,
   source: &TmuxConfigOverride,
) {
...
mod tests {
   ...
   fn test_merge_configs_with_volume_ignores_override() {
      ...
      let merged = merge_configs(global, &profile);
      assert_eq!(merged.sandbox.volume_ignores, vec!["target", "node_modules"]);
   }
   ...
   fn test_tmux_config_override_serialization() {
      ...
      let deserialized: ProfileConfig = toml::from_str(&serialized).unwrap();
      assert_eq!(deserialized.tmux.as_ref().unwrap().mouse, Some(TmuxMouseMode::Enabled));
   }
