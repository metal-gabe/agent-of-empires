...
impl GroupTree {
   ...
   fn build_children(
      ...
      let child_paths: Vec<String> = self
         .insertion_order
         .iter()
         .filter(|p| self.groups_by_path.contains_key(*p) && p.starts_with(&prefix) && !p[prefix.len()..].contains('/'))
         .cloned()
         .collect();
   ...
   pub fn get_sibling_names(
      ...
      if parent_path.is_empty() {
         self
            .groups_by_path
            .keys()
            .filter(|p| !p.contains('/') && p != &path)
            .map(|p| self.groups_by_path.get(p).map(|g| g.name.clone()).unwrap_or_default())
            .collect()
      } else {
         ...
         self
            .groups_by_path
            .keys()
            .filter(|p| p.starts_with(&prefix) && !p[prefix.len()..].contains('/') && p != &path)
            .map(|p| self.groups_by_path.get(p).map(|g| g.name.clone()).unwrap_or_default())
            .collect()
      }
   }
   ...
   pub fn rename_group(
      ...
      if new_leaf.contains('/') {
         anyhow::bail!("Group names cannot contain '/'");
      }

      if !self.groups_by_path.contains_key(old_path) {
         anyhow::bail!("Group '{}' does not exist", old_path);
      }

      let new_path = if let Some(last_slash) = old_path.rfind('/') {
         format!("{}/{}", &old_path[..last_slash], new_leaf)
      } else {
         new_leaf.to_string()
      };

      if new_path != old_path && self.groups_by_path.contains_key(&new_path) {
         anyhow::bail!("A group with the name '{}' already exists", new_leaf);
      }
      ...
      let paths_to_rename: Vec<String> = self
         .groups_by_path
         .keys()
         .filter(|p| *p == old_path || p.starts_with(&prefix))
         .cloned()
         .collect();

      let mut new_entries = Vec::new();
      for old_p in paths_to_rename {
         let new_p = if old_p == old_path {
            new_path.clone()
         } else {
            let suffix = &old_p[prefix.len()..];
            format!("{}/{}", new_path, suffix)
         };

         if let Some(mut group) = self.groups_by_path.remove(&old_p) {
            group.path = new_p.clone();
            if old_p == old_path {
               group.name = new_leaf.to_string();
            }
            new_entries.push((new_p, group));
         }
      }

      for (new_p, group) in new_entries {
         self.groups_by_path.insert(new_p, group);
      }

      for io_path in &mut self.insertion_order {
         if *io_path == old_path {
            *io_path = new_path.clone();
         } else if io_path.starts_with(&prefix) {
            let suffix = &io_path[prefix.len()..];
            *io_path = format!("{}/{}", new_path, suffix);
         }
      }
...
pub fn flatten_tree(
   ...
   let mut ungrouped: Vec<&Instance> = instances.iter().filter(|i| i.group_path.is_empty()).collect();
...
fn flatten_group(
   ...
   let mut group_sessions: Vec<&Instance> = instances.iter().filter(|i| i.group_path == group.path).collect();
   ...
   match sort_order {
      ...
      SortOrder::Newest => {
         children_to_iterate.sort_by_key(|g| Reverse(max_created_at_in_group(&g.path, instances)));
      },
...
mod tests {
   ...
   fn test_collapsed_group_still_shows_in_flatten() {
      ...
      let group_items: Vec<_> = items.iter().filter(|i| matches!(i, Item::Group { .. })).collect();
   ...
   fn test_nested_group_collapse_hides_children() {
      ...
      let items = flatten_tree(&tree, &instances, SortOrder::Oldest);
      let group_count = items.iter().filter(|i| matches!(i, Item::Group { .. })).count();
      assert_eq!(group_count, 2);

      tree.toggle_collapsed("parent");
      let items = flatten_tree(&tree, &instances, SortOrder::Oldest);
      let group_count_collapsed = items.iter().filter(|i| matches!(i, Item::Group { .. })).count();
   ...
   fn test_rename_group_basic() {
      ...
      assert!(tree.group_exists("work"));
      tree.rename_group("work", "jobs").unwrap();
      assert!(!tree.group_exists("work"));
      assert!(tree.group_exists("jobs"));
   }

   #[test]
   fn test_rename_group_updates_children() {
      let mut inst1 = Instance::new("parent", "/tmp/p");
      inst1.group_path = "work".to_string();
      let mut inst2 = Instance::new("child", "/tmp/c");
      inst2.group_path = "work/frontend".to_string();
      let instances = vec![inst1, inst2];
      let mut tree = GroupTree::new_with_groups(&instances, &[]);
      ...
      assert!(!tree.group_exists("work/frontend"));
      assert!(tree.group_exists("jobs"));
      assert!(tree.group_exists("jobs/frontend"));
   }

   #[test]
   fn test_rename_group_case_only() {
      let mut inst = Instance::new("test", "/tmp/t");
      inst.group_path = "Frontend".to_string();
      let instances = vec![inst];
      let mut tree = GroupTree::new_with_groups(&instances, &[]);

      tree.rename_group("Frontend", "frontend").unwrap();
      assert!(tree.group_exists("frontend"));
   }

   #[test]
   fn test_rename_group_duplicate_sibling() {
      let mut inst1 = Instance::new("test1", "/tmp/1");
      inst1.group_path = "work".to_string();
      let mut inst2 = Instance::new("test2", "/tmp/2");
      ...
      let result = tree.rename_group("work", "personal");
      assert!(result.is_err());
      assert_eq!(
         result.unwrap_err().to_string(),
         "A group with the name 'personal' already exists"
      );
   }

   #[test]
   fn test_rename_group_slash_in_name() {
      let mut inst = Instance::new("test", "/tmp/t");
      inst.group_path = "work".to_string();
      let instances = vec![inst];
      let mut tree = GroupTree::new_with_groups(&instances, &[]);

      let result = tree.rename_group("work", "new/name");
      assert!(result.is_err());
      assert_eq!(result.unwrap_err().to_string(), "Group names cannot contain '/'");
   }

   #[test]
   fn test_rename_group_empty_name() {
      let mut inst = Instance::new("test", "/tmp/t");
      inst.group_path = "work".to_string();
      let instances = vec![inst];
      let mut tree = GroupTree::new_with_groups(&instances, &[]);

      let result = tree.rename_group("work", "");
      assert!(result.is_err());
      assert_eq!(result.unwrap_err().to_string(), "Group name cannot be empty");
   }
   ...
   fn test_rename_group_nonexistent() {
      ...
      let result = tree.rename_group("nonexistent", "newname");
      assert!(result.is_err());
      assert_eq!(result.unwrap_err().to_string(), "Group 'nonexistent' does not exist");
   }

   #[test]
   fn test_rename_group_preserves_insertion_order() {
      let mut inst1 = Instance::new("test1", "/tmp/1");
      inst1.group_path = "alpha".to_string();
      let mut inst2 = Instance::new("test2", "/tmp/2");
      inst2.group_path = "work".to_string();
      ...
      let before = tree.get_all_groups();
      let before_names: Vec<_> = before.iter().map(|g| g.name.as_str()).collect();
      assert_eq!(before_names, vec!["alpha", "work", "gamma"]);

      tree.rename_group("work", "jobs").unwrap();

      let after = tree.get_all_groups();
      let after_names: Vec<_> = after.iter().map(|g| g.name.as_str()).collect();
      assert_eq!(after_names, vec!["alpha", "jobs", "gamma"]);
   }

   #[test]
   fn test_rename_nested_group() {
      let mut inst1 = Instance::new("test1", "/tmp/1");
      inst1.group_path = "work/frontend".to_string();
      let instances = vec![inst1];
      let mut tree = GroupTree::new_with_groups(&instances, &[]);
      ...
      tree.rename_group("work/frontend", "ui").unwrap();

      assert!(tree.group_exists("work"));
      assert!(!tree.group_exists("work/frontend"));
      assert!(tree.group_exists("work/ui"));
   }

   #[test]
   fn test_rename_group_preserves_collapsed_state() {
      let mut inst = Instance::new("test", "/tmp/t");
      inst.group_path = "work".to_string();
      let instances = vec![inst];
      let mut tree = GroupTree::new_with_groups(&instances, &[]);
      ...
      tree.toggle_collapsed("work");
      let before = tree.groups_by_path.get("work").unwrap();
      assert!(before.collapsed);

      tree.rename_group("work", "jobs").unwrap();

      let after = tree.groups_by_path.get("jobs").unwrap();
      assert!(after.collapsed);
   }

   #[test]
   fn test_get_sibling_names_root() {
      let mut inst1 = Instance::new("test1", "/tmp/1");
      inst1.group_path = "alpha".to_string();
      let mut inst2 = Instance::new("test2", "/tmp/2");
      inst2.group_path = "work".to_string();
      ...
      let siblings = tree.get_sibling_names("work");
      let mut sibling_names: Vec<_> = siblings.iter().map(|s| s.as_str()).collect();
      sibling_names.sort();
      assert_eq!(sibling_names, vec!["alpha", "gamma"]);
   }

   #[test]
   fn test_get_sibling_names_nested() {
      let mut inst1 = Instance::new("test1", "/tmp/1");
      inst1.group_path = "work/frontend".to_string();
      let mut inst2 = Instance::new("test2", "/tmp/2");
      inst2.group_path = "work/backend".to_string();
      let mut inst3 = Instance::new("test3", "/tmp/3");
      inst3.group_path = "work/devops".to_string();
      let instances = vec![inst1, inst2, inst3];
      let tree = GroupTree::new_with_groups(&instances, &[]);

      let siblings = tree.get_sibling_names("work/frontend");
      let mut sibling_names: Vec<_> = siblings.iter().map(|s| s.as_str()).collect();
      sibling_names.sort();
      assert_eq!(sibling_names, vec!["backend", "devops"]);
   }

   #[test]
   fn test_get_sibling_names_no_siblings() {
      let mut inst = Instance::new("test", "/tmp/t");
      inst.group_path = "work".to_string();
      let instances = vec![inst];
      let tree = GroupTree::new_with_groups(&instances, &[]);
