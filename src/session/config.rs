...
pub fn get_update_settings() -> UpdatesConfig {
   load_config().ok().flatten().map(|c| c.updates).unwrap_or_default()
}
...
mod tests {
   ...
   fn test_sandbox_config_deserialize() {
      ...
      assert_eq!(sb.port_mappings, vec!["3000:3000".to_string(), "5432:5432".to_string()]);
   }
   ...
   fn test_sandbox_config_volume_ignores_roundtrip() {
      ...
      assert_eq!(deserialized.sandbox.volume_ignores, vec!["target", "node_modules"]);
   }
