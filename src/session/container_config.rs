...
/// should be written.
#[cfg(any(target_os = "macos", test))]
fn should_overwrite_credential(
...
#[cfg(target_os = "macos")]
fn extract_keychain_credential(
   service: &str,
   dest: &Path,
) -> Result<bool> {
   ...
   if !output.status.success() {
      ...
      if code == 36 {
         ...
         tracing::debug!("No keychain entry found for service '{}' (account '{}')", service, user);
      } else {
         tracing::warn!(
   ...
   if trimmed.is_empty() {
      tracing::warn!("Keychain entry for service '{}' exists but has empty content", service);
      return Ok(false);
   }
   ...
   tracing::debug!("Extracted keychain credential for '{}' -> {}", service, dest.display());
   Ok(true)
}
   ...
   if host_dir.exists() {
      ...
      if let Some((service, filename)) = mount.keychain_credential {
         if let Err(e) = extract_keychain_credential(service, &sandbox_dir.join(filename)) {
            tracing::warn!("Failed to extract keychain credential for {}: {}", mount.host_rel, e);
         }
      }
   } else {
      std::fs::create_dir_all(&sandbox_dir)?;
...
pub(crate) fn compute_volume_paths(
   ...
   if project_path.join(".git").exists() {
      if let Ok(main_repo) = GitWorktree::find_main_repo(project_path) {
         ...
         let main_repo_canonical = main_repo.canonicalize().unwrap_or_else(|_| main_repo.clone());
         ...
         if main_repo_canonical != project_canonical {
            if project_canonical.starts_with(&main_repo_canonical) {
               ...
               let wt_rel = project_canonical.strip_prefix(&common).unwrap_or(&project_canonical);
...
pub(crate) fn refresh_agent_configs() {
   ...
   for mount in AGENT_CONFIG_MOUNTS {
      if let Err(e) = prepare_sandbox_dir(mount, &home) {
         tracing::warn!("Failed to refresh agent config for {}: {}", mount.host_rel, e);
      }
   }
}
...
pub(crate) fn build_container_config(
   ...
   for mount in AGENT_CONFIG_MOUNTS.iter().filter(|m| m.tool_name == tool) {
      ...
      let sandbox_dir = match prepare_sandbox_dir(mount, &home) {
         Ok(dir) => dir,
         Err(e) => {
            tracing::warn!("Failed to prepare sandbox dir for {}, skipping: {}", mount.host_rel, e);
            continue;
         },
      };
   ...
   if let Some(agent) = crate::agents::get_agent(tool) {
      if let Some(hook_cfg) = &agent.hook_config {
         ...
         if let Err(e) = std::fs::create_dir_all(&hook_dir) {
            tracing::warn!("Failed to create hook directory {}: {}", hook_dir.display(), e);
         }
         volumes.push(VolumeMount {
   ...
   tracing::debug!("extra_volumes from config: {:?}", sandbox_config.extra_volumes);
   let mut extra_volume_container_paths: std::collections::HashSet<String> = std::collections::HashSet::new();
...
mod tests {
   ...
   fn test_compute_volume_paths_regular_repo() {
      ...
      assert_eq!(volumes[0].host_path, repo_path.to_string_lossy().to_string());
      ...
      assert_eq!(volumes[0].container_path, format!("/workspace/{}", dir_name));
   }
   ...
   fn test_compute_volume_paths_non_git_directory() {
      ...
      assert_eq!(volumes[0].host_path, dir.path().to_string_lossy().to_string());
      // Container path and working dir should be the same
      assert_eq!(volumes[0].container_path, working_dir);
   }
   ...
   fn test_compute_volume_paths_bare_repo_worktree() {
      ...
      let (volumes, working_dir) = compute_volume_paths(&worktree_path, project_path_str).unwrap();
   ...
   fn test_compute_volume_paths_non_bare_repo_worktree() {
      ...
      repo
         .branch("wt-branch", &repo.find_commit(head).unwrap(), false)
         .unwrap();
      drop(repo);

      let output = std::process::Command::new("git")
         .args(["worktree", "add", worktree_path.to_str().unwrap(), "wt-branch"])
      ...
      assert_eq!(volumes.len(), 2, "Should have two volumes: main repo and worktree");
      ...
      assert_eq!(mount0_canon, repo_canon, "First volume should mount the main repo");
      let repo_name = repo_canon.file_name().unwrap().to_string_lossy();
      assert_eq!(volumes[0].container_path, format!("/workspace/{}", repo_name),);
      ...
      assert_eq!(mount1_canon, wt_canon, "Second volume should mount the worktree");
   ...
   fn test_compute_volume_paths_bare_repo_root() {
      ...
      let (volumes, working_dir) = compute_volume_paths(&main_repo_path, project_path_str).unwrap();
   ...
   fn test_compute_volume_paths_non_bare_worktree_nested_layout() {
      ...
      let expected_repo = format!("/workspace/{}", repo_canon.strip_prefix(&common).unwrap().display());
      let expected_wt = format!("/workspace/{}", wt_canon.strip_prefix(&common).unwrap().display());
      ...
      assert!(
         normalized.to_string_lossy().starts_with(&volumes[0].container_path),
   ...
   fn test_agent_config_mounts_filter_by_tool() {
      let claude_mounts: Vec<_> = AGENT_CONFIG_MOUNTS.iter().filter(|m| m.tool_name == "claude").collect();
      assert_eq!(claude_mounts.len(), 1);
      assert_eq!(claude_mounts[0].host_rel, ".claude");

      let cursor_mounts: Vec<_> = AGENT_CONFIG_MOUNTS.iter().filter(|m| m.tool_name == "cursor").collect();
   ...
   fn test_refresh_picks_up_new_host_files() {
      ...
      sync_agent_config(&host, &sandbox, &[], &[], &[], &[]).unwrap();
      assert_eq!(fs::read_to_string(sandbox.join("new_cred.json")).unwrap(), "new");
   }
   ...
   fn test_copies_listed_dirs_recursively() {
      ...
      assert!(sandbox.join("plugins").join("lsp").join("gopls.wasm").exists());
   ...
   fn test_copy_dir_recursive() {
      ...
      assert_eq!(fs::read_to_string(dest.join("a").join("mid.txt")).unwrap(), "mid");
      assert_eq!(
   ...
   fn test_preserve_files_not_overwritten() {
      ...
      sync_agent_config(&host, &sandbox, &[], &[], &[], &["auth.json"]).unwrap();
      assert_eq!(fs::read_to_string(sandbox.join("settings.json")).unwrap(), "updated");
   }
   ...
   fn test_has_prior_data_skips_general_file_copy() {
      ...
      sync_agent_config(&host, &sandbox, &[], &[], &[], &[]).unwrap();
      assert_eq!(fs::read_to_string(sandbox.join("settings.json")).unwrap(), "{}");
