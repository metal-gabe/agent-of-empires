...
impl Instance {
   ...
   pub fn get_tool_command(&self) -> &str {
      if self.command.is_empty() {
         crate::agents::get_agent(&self.tool).map(|a| a.binary).unwrap_or("bash")
      } else {
         &self.command
      }
   }
   ...
   pub fn has_terminal(&self) -> bool {
      self.terminal_info.as_ref().map(|t| t.created).unwrap_or(false)
   }
   ...
   pub fn start_container_terminal_with_size(
      &mut self,
      size: Option<(u16, u16)>,
   ) -> Result<()> {
      ...
      let cmd = container.exec_command(Some(&format!("-w {} {}", container_workdir, env_part)), "/bin/bash");
   ...
   fn apply_session_tmux_options(
      ...
      crate::tmux::status_bar::apply_all_tmux_options(session_name, display_title, branch, sandbox.as_ref());
   }
   ...
   pub fn start_with_size_opts(
      ...
      let on_launch_hooks = if skip_on_launch {
         ...
         match super::repo_config::check_hook_trust(Path::new(&self.project_path)) {
            Ok(super::repo_config::HookTrustStatus::Trusted(hooks)) if !hooks.on_launch.is_empty() => {
      ...
      let cmd = if self.is_sandboxed() {
         ...
         if let Some(ref hook_cmds) = on_launch_hooks {
            if let Some(ref sandbox) = self.sandbox_info {
               ...
               if let Err(e) =
                  super::repo_config::execute_hooks_in_container(hook_cmds, &sandbox.container_name, &workdir)
               {
         ...
         let mut tool_cmd = if self.is_yolo_mode() {
            if let Some(ref yolo) = agent.and_then(|a| a.yolo.as_ref()) {
               match yolo {
                  ...
                  crate::agents::YoloMode::EnvVar(..) | crate::agents::YoloMode::AlwaysYolo => base_cmd,
               }
            } else {
               base_cmd
            }
         ...
         if let Some(ref hook_cmds) = on_launch_hooks {
            if let Err(e) = super::repo_config::execute_hooks(hook_cmds, Path::new(&self.project_path)) {
...
mod tests {
   ...
   fn test_worktree_info_default_cleanup_on_delete() {
      ...
      let json =
         r#"{"branch":"test","main_repo_path":"/path","managed_by_aoe":true,"created_at":"2024-01-01T00:00:00Z"}"#;
