...
use super::profile_config::{
   HooksConfigOverride, ProfileConfig, SandboxConfigOverride, SessionConfigOverride, TmuxConfigOverride,
...
pub fn save_repo_config(
   ...
   if !aoe_dir.exists() {
      fs::create_dir_all(&aoe_dir).with_context(|| format!("Failed to create {}", aoe_dir.display()))?;
   }

   let config_path = project_path.join(REPO_CONFIG_PATH);
   let content = toml::to_string_pretty(config).with_context(|| "Failed to serialize repo config".to_string())?;
...
pub fn merge_repo_config(
   ...
   use super::profile_config::{
      apply_sandbox_overrides, apply_session_overrides, apply_tmux_overrides, apply_worktree_overrides,
   };
...
pub fn resolve_config_with_repo(
   profile: &str,
   project_path: &Path,
) -> Result<Config> {
...
pub enum HookTrustStatus {
   ...
   NeedsTrust { hooks: HooksConfig, hooks_hash: String },
}
...
enum HookTarget<'a> {
   ...
   Container { container_name: &'a str, workdir: &'a str },
}
...
fn build_hook_command(
   ...
   match target {
      ...
      HookTarget::Container {
         ...
         command.args(["exec", "--workdir", workdir, container_name, "bash", "-c", &shell_cmd]);
         command
      },
   }
}
...
fn format_hook_error(
   ...
   let mut detail = format!("{} with exit code {}: {}", prefix, exit_code.unwrap_or(-1), cmd);
   if !stderr.is_empty() {
...
mod tests {
   ...
   fn test_repo_config_deserialization() {
      ...
      assert_eq!(config.session.unwrap().default_tool, Some("opencode".to_string()));
   ...
   fn test_merge_repo_config_no_overrides() {
      ...
      assert_eq!(merged.sandbox.enabled_by_default, config.sandbox.enabled_by_default);
   }
   ...
   fn test_normalize_path_real_dir() {
      ...
      assert_eq!(std::fs::canonicalize(tmp.path()).unwrap().to_string_lossy(), normalized);
   }
