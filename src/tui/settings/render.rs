...
impl SettingsView {
   ...
   fn render_header(
      ...
      let profile_label = if self.scope == SettingsScope::Profile && self.available_profiles.len() > 1 {
   ...
   fn render_fields(
      ...
      if total_content_height > fields_viewport_height {
         ...
         let mut scrollbar_state =
            ScrollbarState::new(total_content_height.saturating_sub(fields_viewport_height) as usize)
   ...
   fn render_field(
      ...
      let label = Line::from(vec![Span::styled(field.label, label_style), override_indicator]);
      ...
      let value_area = Rect { y: area.y + 1, ..area };
   ...
   fn render_bool_field(
      ...
      let text = format!("{} {}", checkbox, if value { "Enabled" } else { "Disabled" });
   ...
   fn render_list_field(
      ...
      if !is_expanded {
         ...
         for (i, item) in items.iter().enumerate() {
            ...
            let prefix = if i == list_state.selected_index { "> " } else { "  " };
      ...
      let spans: Vec<Span> = if self.custom_instruction_dialog.is_some() {
         ...
         match self.focus {
            ...
            SettingsFocus::Fields => {
               ...
               {
                  s.extend([Span::styled("r", key_style), Span::styled(": reset  ", desc_style)]);
               }
            },
         }

         s.extend([Span::styled("[]", key_style), Span::styled(": scope  ", desc_style)]);

         if self.scope == SettingsScope::Profile && self.available_profiles.len() > 1 {
            s.extend([Span::styled("{}", key_style), Span::styled(": profile  ", desc_style)]);
         }
