...
impl SettingsView {
      ...
      match (key.code, key.modifiers) {
               ...
               match &field.value {
                  ...
                  FieldValue::OptionalText(value) => {
                     if field.key == FieldKey::CustomInstruction {
                        self.custom_instruction_dialog = Some(CustomInstructionDialog::new(value.clone()));
                     } else {
                        self.editing_input = Some(Input::new(value.clone().unwrap_or_default()));
                     }
                  },
                  ...
                  FieldValue::Select { selected, options } => {
                     ...
                     if self.fields[self.selected_field].key == FieldKey::ThemeName {
                        if let FieldValue::Select { selected, options } = &self.fields[self.selected_field].value {
         ...
         (KeyCode::Char('r'), _) => {
               ...
               && !self.fields.is_empty()
            {
               ...
               if was_theme {
                  if let Some(field) = self.fields.iter().find(|f| f.key == FieldKey::ThemeName) {
            ...
            if let Some(input) = self.editing_input.take() {
               ...
               if let Err(e) = field.validate() {
                  ...
                  self.editing_input = match &field.value {
                     ...
                     FieldValue::OptionalText(v) => Some(Input::new(v.clone().unwrap_or_default())),
   ...
   fn clear_profile_override(
      ...
      if self.scope == SettingsScope::Repo {
         self.repo_config = Some(crate::session::profile_to_repo_config(&self.repo_as_profile));
      }
   ...
   pub fn discard_changes(&mut self) -> anyhow::Result<()> {
      ...
      self.repo_config = self
         .project_path
         .as_ref()
         .and_then(|p| crate::session::load_repo_config(std::path::Path::new(p)).ok().flatten());
      self.resolved_base = crate::session::merge_configs(self.global_config.clone(), &self.profile_config);
