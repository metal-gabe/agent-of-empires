...
use crate::session::{
   list_profiles, load_profile_config, load_repo_config, merge_configs, profile_to_repo_config, repo_config_to_profile,
   save_config, save_profile_config, save_repo_config, Config, ProfileConfig, RepoConfig,
};
...
impl SettingsView {
   pub fn new(
      ...
      let repo_as_profile = repo_config.as_ref().map(repo_config_to_profile).unwrap_or_default();
   ...
   pub(super) fn rebuild_fields(&mut self) {
      ...
      let (scope_for_fields, global_ref, profile_ref) = match self.scope {
         SettingsScope::Global => (SettingsScope::Global, &self.global_config, &self.profile_config),
         SettingsScope::Profile => (SettingsScope::Profile, &self.global_config, &self.profile_config),
         SettingsScope::Repo => (SettingsScope::Profile, &self.resolved_base, &self.repo_as_profile),
      };
      self.fields = fields::build_fields_for_category(category, scope_for_fields, global_ref, profile_ref);
   ...
   pub(super) fn ensure_field_visible(
      &mut self,
      viewport_height: u16,
   ) {
      let mut y = 0u16;
   ...
   pub(super) fn apply_field_to_config(
      &mut self,
      field_index: usize,
   ) {
      ...
      match self.scope {
         SettingsScope::Global | SettingsScope::Profile => {
            fields::apply_field_to_config(field, self.scope, &mut self.global_config, &mut self.profile_config);
         },
         SettingsScope::Repo => {
   ...
   pub fn save(&mut self) -> anyhow::Result<()> {
      ...
      match self.scope {
         ...
         SettingsScope::Repo => {
            if let (Some(ref project_path), Some(ref repo_config)) = (&self.project_path, &self.repo_config) {
   ...
   pub fn is_editing(&self) -> bool {
      self.editing_input.is_some() || self.list_edit_state.is_some() || self.custom_instruction_dialog.is_some()
   }
}
