   ...
   pub fn render_terminal_preview(
      ...
      let mut info_lines = vec![
         ...
         Line::from(vec![
            ...
            Span::styled(
               if terminal_running { "Running" } else { "Not started" },
      ...
      if !terminal_running {
         ...
         let output_lines: Vec<Line> = cached_output.lines().map(|line| Line::from(Span::raw(line))).collect();
   ...
   fn render_info(
      ...
      if let Some(wt_info) = &instance.worktree_info {
         ...
         info_lines.push(Line::from(vec![
            ...
            Span::styled(shorten_path(&wt_info.main_repo_path), Style::default().fg(theme.text)),
         ]));
   ...
   fn render_output_cached(
      ...
      if let Some(error) = &instance.last_error {
         let error_lines: Vec<Line> = vec![
            Line::from(Span::styled("Error:", Style::default().fg(theme.error).bold())),
            Line::from(""),
...
fn shorten_path(path: &str) -> String {
   ...
   if let Some(home) = dirs::home_dir() {
      if let (Ok(canonical_path), Ok(canonical_home)) = (path_buf.canonicalize(), home.canonicalize()) {
