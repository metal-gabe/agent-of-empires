...
use ratatui::{
   ...
   text::{Line, Span},
   widgets::{
      Block, Borders, Clear, List, ListItem, Padding, Paragraph, Scrollbar, ScrollbarOrientation, ScrollbarState,
   },
   Frame,
};
...
impl DiffView {
   ...
   fn render_header(
      ...
      let repo_name = self.repo_path.file_name().and_then(|n| n.to_str()).unwrap_or("repo");
      ...
      let header = Line::from(vec![
         ...
         Span::styled(format!("{} changed", file_count), Style::default().fg(theme.dimmed)),
         Span::styled("  ", Style::default()),
         Span::styled(format!("+{}", additions), Style::default().fg(theme.diff_add)),
         Span::styled(" ", Style::default()),
         Span::styled(format!("-{}", deletions), Style::default().fg(theme.diff_delete)),
      ]);
   ...
   fn render_content(
      ...
      let layout = Layout::default()
         ...
         .constraints([Constraint::Length(self.file_list_width), Constraint::Min(40)])
         .split(area);
   ...
   fn render_diff_content(
      ...
      if let Some(file) = self.files.get(self.selected_file) {
         if let Some(diff) = self.diff_cache.get(&file.path) {
            if diff.is_binary {
               let msg = Paragraph::new("Binary file").style(Style::default().fg(theme.dimmed));
               frame.render_widget(msg, inner);
               return;
            }
            ...
            for hunk in &diff.hunks {
               ...
               lines.push(Line::from(Span::styled(header, Style::default().fg(theme.diff_header))));
               ...
               for line in &hunk.lines {
                  ...
                  lines.push(Line::from(vec![
                     Span::styled(format!("{} {} ", old_num, new_num), Style::default().fg(theme.dimmed)),
                     Span::styled(prefix, style),
            ...
            let msg = Paragraph::new("Loading diff...").style(Style::default().fg(theme.dimmed));
            frame.render_widget(msg, inner);
         }
      } else {
   ...
   fn render_help(
      ...
      let shortcuts = vec![
         ...
         ("Other", vec![("?", "Toggle this help"), ("q/Esc", "Close diff view")]),
      ];
