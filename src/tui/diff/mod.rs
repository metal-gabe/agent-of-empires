...
use crate::git::diff::{
   check_merge_base_status, compute_changed_files, compute_file_diff, get_default_branch, list_branches, DiffFile,
   FileDiff,
};
...
impl DiffView {
   ...
   pub fn new(repo_path: PathBuf) -> anyhow::Result<Self> {
      ...
      let warning_dialog =
         check_merge_base_status(&repo_path, &base_branch).map(|msg| InfoDialog::new("Warning", &msg));
   ...
   pub fn get_current_diff(&mut self) -> Option<&FileDiff> {
      ...
      if !self.diff_cache.contains_key(&path) {
         match compute_file_diff(&self.repo_path, &path, &self.base_branch, self.context_lines) {
            Ok(diff) => {
   ...
   pub fn open_branch_select(&mut self) {
      match list_branches(&self.repo_path) {
         Ok(branches) => {
            let selected = branches.iter().position(|b| b == &self.base_branch).unwrap_or(0);
   ...
   pub fn select_branch(
      ...
      self.warning_dialog =
         check_merge_base_status(&self.repo_path, &self.base_branch).map(|msg| InfoDialog::new("Warning", &msg));
