...
pub fn check_version_change() -> Result<Option<String>> {
   ...
   if config.app_state.has_seen_welcome && config.app_state.last_seen_version.as_deref() != Some(current_version) {
...
impl App {
   ...
   fn poll_update_check(&mut self) -> bool {
      let (update_info, update_rx, received) = poll_update_receiver(self.update_rx.take(), self.update_info.take());
...
fn poll_update_receiver(
   ...
   if let Some(mut rx) = rx {
      match rx.try_recv() {
         ...
         Err(tokio::sync::oneshot::error::TryRecvError::Empty) => (current_info, Some(rx), false),
...
impl App {
      ...
      match action {
         ...
         Action::StopSession(id) => {
            if let Some(inst) = self.home.get_instance(&id) {
               ...
               // can take up to 10s).
               self.home.set_instance_status(&id, crate::session::Status::Stopped);
               match inst_clone.stop() {
                  ...
                  Err(e) => {
                     ...
                     self.home.set_instance_status(&id, crate::session::Status::Error);
                     self.home.save()?;
                  },
               }
            }
   ...
   fn attach_session(
         ...
         || (!instance.expects_shell() && tmux_session.is_pane_running_shell())
      {
         ...
         if instance.is_sandboxed() {
            ...
            if has_instruction
               && !crate::agents::get_agent(&instance.tool).is_some_and(|a| a.instruction_flag.is_some())
            {
               let config = load_config()?.unwrap_or_default();
         ...
         if let Err(e) = inst.start_with_size_opts(size, skip_on_launch) {
            self.home.set_instance_error(session_id, Some(e.to_string()));
            self.home.set_instance_status(session_id, crate::session::Status::Error);
            return Ok(());
         }
      ...
      let attach_fn: Box<dyn FnOnce() -> Result<()>> = match mode {
         ...
         _ => {
            ...
            if !terminal_session.exists() || terminal_session.is_pane_dead() {
               ...
               if let Err(e) = self.home.start_terminal_for_instance_with_size(session_id, size) {
                  self.home.set_instance_error(session_id, Some(e.to_string()));
                  return Ok(());
               }
            }
   ...
   fn edit_file(
      ...
      let status = with_raw_mode_disabled(terminal, move || {
         std::process::Command::new(&editor_clone).arg(&path).status()
      })?;
...
mod tests {
   ...
   fn test_action_enum() {
      ...
      let attach_terminal = Action::AttachTerminal("test-id".to_string(), TerminalMode::Container);
