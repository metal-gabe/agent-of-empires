...
use super::{
   get_indent, HomeView, TerminalMode, ViewMode, ICON_COLLAPSED, ICON_DELETING, ICON_ERROR, ICON_EXPANDED, ICON_IDLE,
...
impl HomeView {
   ...
   fn render_list(
      ...
      let list_items: Vec<ListItem> = self
         ...
         .map(|(idx, item)| {
            ...
            let is_match = !self.search_matches.is_empty() && self.search_matches.contains(&idx);
      ...
      if self.search_active {
         ...
         if !self.search_matches.is_empty() {
            let count_text = format!(" [{}/{}]", self.search_match_index + 1, self.search_matches.len());
                  ...
                  ViewMode::Terminal => {
                     ...
                     let terminal_running = match terminal_mode {
                        TerminalMode::Container => inst
                           ...
                           .map(|s| s.exists())
                           .unwrap_or(false),
                        TerminalMode::Host => inst.terminal_tmux_session().map(|s| s.exists()).unwrap_or(false),
                     };
      ...
      if let Item::Session { id, .. } = item {
         if let Some(inst) = self.get_instance(id) {
            ...
            if inst.is_sandboxed() {
               match self.view_mode {
                  ViewMode::Agent => {
                     line_spans.push(Span::styled(" [sandbox]", Style::default().fg(theme.sandbox)));
                  },
                  ViewMode::Terminal => {
   ...
   fn refresh_preview_cache_if_needed(
      &mut self,
      width: u16,
      height: u16,
   ) {
   ...
   fn refresh_terminal_preview_cache_if_needed(
      &mut self,
      width: u16,
      height: u16,
   ) {
      ...
      let needs_refresh = match &self.selected_session {
         Some(id) => {
            self.terminal_preview_cache.session_id.as_ref() != Some(id)
               ...
               || self.terminal_preview_cache.last_refresh.elapsed().as_millis() > PREVIEW_REFRESH_MS
         },
         None => false,
      };
   ...
   fn refresh_container_terminal_preview_cache_if_needed(
      &mut self,
      width: u16,
      height: u16,
   ) {
      ...
      let needs_refresh = match &self.selected_session {
         Some(id) => {
            self.container_terminal_preview_cache.session_id.as_ref() != Some(id)
               ...
               || self.container_terminal_preview_cache.last_refresh.elapsed().as_millis() > PREVIEW_REFRESH_MS
         },
         None => false,
      };
            ...
            if let Some(id) = selected_id {
               ...
               match terminal_mode {
                  TerminalMode::Container => {
                     self.refresh_container_terminal_preview_cache_if_needed(inner.width, inner.height);
                  },
                  TerminalMode::Host => {
               ...
               if let Some(inst) = self.get_instance(&id) {
                  let (terminal_running, preview_content) = match terminal_mode {
                     ...
                     TerminalMode::Host => {
                        let running = inst.terminal_tmux_session().map(|s| s.exists()).unwrap_or(false);
                        (running, &self.terminal_preview_cache.content)
                     },
                  };

                  Preview::render_terminal_preview(frame, inner, inst, terminal_running, preview_content, theme);
               }
            } else {
   ...
   fn render_status_bar(
      ...
      if let Some(enter_action_text) = match self.flat_items.get(self.cursor) {
         Some(Item::Group { collapsed: true, .. }) => Some(" Expand "),
         Some(Item::Group { collapsed: false, .. }) => Some(" Collapse "),
   ...
   fn render_update_bar(
      ...
      let text = format!(" update available {} -> {}", info.current_version, info.latest_version);
      let bar =
         Paragraph::new(Line::from(Span::styled(text, update_style))).style(Style::default().bg(theme.selection));
