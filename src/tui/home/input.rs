...
use crate::tui::dialogs::{
   ConfirmDialog, DeleteDialogConfig, DialogResult, GroupDeleteOptionsDialog, GroupRenameDialog, HookTrustAction,
   InfoDialog, NewSessionData, NewSessionDialog, ProfilePickerAction, RenameDialog, UnifiedDeleteDialog,
};
...
impl HomeView {
   pub fn handle_key(
               ...
               if let Some(data) = self.pending_hook_trust_data.take() {
                  match action {
                     HookTrustAction::Trust {
                        ...
                        if let Err(e) = repo_config::trust_repo(std::path::Path::new(&project_path), &hooks_hash) {
                           tracing::error!("Failed to trust repo: {}", e);
                        }
                        let merged = self.merge_repo_hooks_onto_config_for(&data.profile, hooks);
      ...
      let dialog_result = self.new_dialog.as_mut().map(|dialog| dialog.handle_key(key));
            ...
            DialogResult::Submit(data) => {
               ...
               match repo_config::check_hook_trust(std::path::Path::new(&data.path)) {
                  Ok(repo_config::HookTrustStatus::NeedsTrust { hooks, hooks_hash }) => {
                     ...
                     self.hook_trust_dialog = Some(HookTrustDialog::new(hooks, hooks_hash, data.path.clone()));
      ...
      if let Some(dialog) = &mut self.rename_dialog {
         match dialog.handle_key(key) {
            ...
            DialogResult::Submit(data) => {
               self.rename_dialog = None;
               if let Err(e) = self.rename_selected(&data.title, data.group.as_deref(), data.profile.as_deref()) {
      ...
      if let Some(dialog) = &mut self.group_rename_dialog {
         match dialog.handle_key(key) {
            ...
            DialogResult::Submit(new_name) => {
               self.group_rename_dialog = None;
               if let Err(e) = self.rename_selected_group(&new_name) {
            ...
            DialogResult::Submit(action) => match action {
               ...
               ProfilePickerAction::Created(name) => {
                  ...
                  match crate::session::create_profile(&name) {
                     ...
                     Err(e) => {
                        self.info_dialog = Some(InfoDialog::new("Error", &format!("Failed to create profile: {}", e)));
                     },
                  }
               },
               ProfilePickerAction::Deleted(name) => match crate::session::delete_profile(&name) {
                  Ok(()) => {
                     self.show_profile_picker();
                  },
                  Err(e) => {
                     self.profile_picker_dialog = None;
                     self.info_dialog = Some(InfoDialog::new("Error", &format!("Failed to delete profile: {}", e)));
                  },
               },
            },
         }
      ...
      if self.search_active {
         match key.code {
            ...
            _ => {
               self.search_query.handle_event(&crossterm::event::Event::Key(key));
               self.update_search();
            },
         }
         return None;
      ...
      match key.code {
         ...
         KeyCode::Char('s') => {
            ...
            match SettingsView::new(self.storage.profile(), project_path) {
               ...
               Err(e) => {
                  ...
                  self.info_dialog = Some(InfoDialog::new("Error", &format!("Failed to open settings: {}", e)));
               },
            }
         },
         KeyCode::Char('D') => {
            ...
            match DiffView::new(repo_path) {
               ...
               Err(e) => {
                  ...
                  self.info_dialog = Some(InfoDialog::new("Error", &format!("Failed to open diff view: {}", e)));
               },
            }
         },
         KeyCode::Char('x') => {
            if let Some(session_id) = &self.selected_session {
               if let Some(inst) = self.get_instance(session_id) {
                  ...
                  self.confirm_dialog = Some(ConfirmDialog::new("Stop Session", &message, "stop_session"));
               }
            }
         },
         KeyCode::Char('d') => {
            ...
            if let Some(session_id) = &self.selected_session {
               if let Some(inst) = self.get_instance(session_id) {
                  ...
                  self.unified_delete_dialog = Some(UnifiedDeleteDialog::new(inst.title.clone(), config));
               } else {
               ...
               if session_count > 0 {
                  ...
                  self.confirm_dialog = Some(ConfirmDialog::new("Delete Group", &message, "delete_group"));
               }
            }
         },
         KeyCode::Char('r') if !key.modifiers.contains(KeyModifiers::SHIFT) => {
            if let Some(id) = &self.selected_session {
               ...
               let current_name = group_path.split('/').next_back().unwrap_or(group_path).to_string();
               let siblings = self.group_tree.get_sibling_names(group_path);
               self.group_rename_dialog = Some(GroupRenameDialog::new(&current_name, siblings));
            }
         },
         ...
         KeyCode::Left | KeyCode::Char('h') => {
            if let Some(Item::Group { path, collapsed, .. }) = self.flat_items.get(self.cursor) {
               if !collapsed {
                  let path = path.clone();
                  self.toggle_group_collapsed(&path);
               }
