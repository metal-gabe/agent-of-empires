...
impl HomeView {
      ...
      if let Some(group_path) = self.selected_group.take() {
         ...
         for session_id in sessions_to_delete {
            ...
            if let Some(inst) = self.get_instance(&session_id) {
               let delete_worktree =
                  options.delete_worktrees && inst.worktree_info.as_ref().is_some_and(|wt| wt.managed_by_aoe);
               let delete_branch =
                  options.delete_branches && inst.worktree_info.as_ref().is_some_and(|wt| wt.managed_by_aoe);
               let delete_sandbox = options.delete_containers && inst.sandbox_info.as_ref().is_some_and(|s| s.enabled);
               let request = DeletionRequest {
   ...
   pub(super) fn rename_selected_group(
      ...
      if let Some(old_path) = &self.selected_group {
         let old_path = old_path.clone();

         self.group_tree.rename_group(&old_path, new_leaf)?;
         ...
         for inst in &mut self.instances {
            if inst.group_path == old_path {
               inst.group_path = new_path.clone();
            } else if inst.group_path.starts_with(&prefix) {
               let suffix = &inst.group_path[prefix.len()..];
         ...
         let new_cursor = self
            .flat_items
            .iter()
            .position(|item| matches!(item, super::Item::Group { path, .. } if path == &new_path))
