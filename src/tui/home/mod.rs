...
use super::dialogs::{
   ChangelogDialog, ConfirmDialog, GroupDeleteOptionsDialog, GroupRenameDialog, HookTrustDialog, InfoDialog,
   NewSessionData, NewSessionDialog, ProfilePickerDialog, RenameDialog, UnifiedDeleteDialog, WelcomeDialog,
};
...
impl HomeView {
   pub fn new(
      ...
      let instance_map: HashMap<String, Instance> = instances.iter().map(|i| (i.id.clone(), i.clone())).collect();
      ...
      let default_terminal_mode = resolved
         ...
         .unwrap_or_default();
      let sound_config = resolved.as_ref().map(|config| config.sound.clone()).unwrap_or_default();
      ...
      let mut view = Self {
         ...
         diff_view: None,
         list_width: user_config.and_then(|c| c.app_state.home_list_width).unwrap_or(35),
      };
   ...
   pub fn reload(&mut self) -> anyhow::Result<()> {
      ...
      self.instance_map = self.instances.iter().map(|i| (i.id.clone(), i.clone())).collect();
      self.groups = groups;
   ...
   pub fn apply_status_updates(&mut self) -> bool {
      ...
      if let Some(updates) = self.status_poller.try_recv_updates() {
         for update in updates {
            ...
            let should_update = old_status
               .is_some_and(|s| s != Status::Deleting && s != Status::Stopped && update.status != Status::Stopped);
               ...
               match Storage::new(&target_profile) {
                  Ok(target_storage) => match target_storage.load_with_groups() {
                     Ok((mut target_instances, target_groups)) => {
                        ...
                        if let Err(e) = target_storage.save_with_groups(&target_instances, &target_tree) {
   ...
   pub fn take_on_launch_hooks_ran(
      &mut self,
      session_id: &str,
   ) -> bool {
   ...
   pub fn save(&self) -> anyhow::Result<()> {
      self.storage.save_with_groups(&self.instances, &self.group_tree)?;
      Ok(())
   }
