...
impl UnifiedDeleteDialog {
   pub fn new(
      ...
      let options = DeleteOptions {
         ...
         force_delete: false,
         delete_branch: config.worktree_branch.is_some() && user_config.worktree.delete_branch_on_cleanup,
   ...
   pub fn render(
      ...
      let checkbox_count = if has_worktree { 2 } else { 0 } + (show_force as u16) + (has_sandbox as u16);

      let dialog_width = 55;
      let dialog_height = if checkbox_count > 0 { 8 + checkbox_count } else { 7 };
