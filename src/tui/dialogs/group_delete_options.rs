...
impl GroupDeleteOptionsDialog {
   ...
   fn force_field_index(&self) -> Option<usize> {
      if self.options.delete_sessions && self.has_managed_worktrees && self.options.delete_worktrees {
         Some(3)
      } else {
         None
      }
   ...
   pub fn handle_key(
      &mut self,
      key: KeyEvent,
   ) -> DialogResult<GroupDeleteOptions> {
      match key.code {
   ...
   pub fn render(
      ...
      // Group info
      let session_word = if self.session_count == 1 { "session" } else { "sessions" };
      ...
      if show_worktree_option {
         ...
         let wt_checkbox = if self.options.delete_worktrees { "[x]" } else { "[ ]" };
         ...
         let br_checkbox = if self.options.delete_branches { "[x]" } else { "[ ]" };
      ...
      if show_container_option {
         ...
         let ct_checkbox = if self.options.delete_containers { "[x]" } else { "[ ]" };
