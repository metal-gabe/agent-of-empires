...
impl WelcomeDialog {
   ...
   pub fn render(
      ...
      let content = vec![
         ...
         Line::from(""),
         Line::from(vec![
            Span::styled("  Ctrl+b then d   ", Style::default().fg(theme.title).bold()),
            Span::styled("Detach (exit without stopping)", Style::default().fg(theme.text)),
         ]),
         Line::from(vec![
            Span::styled("  Ctrl+b then [   ", Style::default().fg(theme.title).bold()),
            Span::styled(
      ...
      frame.render_widget(Paragraph::new(button).alignment(Alignment::Center), chunks[1]);
   }
}
