...
impl HookTrustDialog {
   ...
   pub fn handle_key(
      ...
      match key.code {
         ...
         KeyCode::Char('y') | KeyCode::Char('Y') => DialogResult::Submit(HookTrustAction::Trust {
            hooks: self.hooks.clone(),
            hooks_hash: self.hooks_hash.clone(),
            project_path: self.project_path.clone(),
         }),
   ...
   fn build_hook_lines(&self) -> Vec<Line<'_>> {
      ...
      if !self.hooks.on_create.is_empty() {
         lines.push(Line::from(Span::styled("on_create:", Style::default().bold())));
      ...
      if !self.hooks.on_launch.is_empty() {
         lines.push(Line::from(Span::styled("on_launch:", Style::default().bold())));
   ...
   pub fn render(
      ...
      // Header
      let header = Paragraph::new("This repo has hooks defined in .aoe/config.toml.\nAllow these commands to run?")
         .style(Style::default().fg(theme.text))
         .wrap(Wrap { trim: true });
      frame.render_widget(header, chunks[0]);

      // Hook commands (scrollable)
      let visible_lines: Vec<Line> = hook_lines.into_iter().skip(self.scroll_offset as usize).collect();
      ...
      frame.render_widget(Paragraph::new(buttons).alignment(Alignment::Center), chunks[2]);
   }
}
...
mod tests {
   ...
   fn test_y_trusts() {
      ...
      assert!(matches!(result, DialogResult::Submit(HookTrustAction::Trust { .. })));
   }
   ...
   fn test_n_skips() {
      ...
      assert!(matches!(result, DialogResult::Submit(HookTrustAction::Skip)));
   }
