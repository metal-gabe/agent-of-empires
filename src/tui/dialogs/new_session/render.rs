...
impl NewSessionDialog {
   pub fn render(
      ...
      if has_tool_selection {
         ...
         let has_config = !self.extra_args.value().is_empty() || !self.command_override.value().is_empty();
         if has_config {
            tool_spans.push(Span::styled("  (configured)", Style::default().fg(theme.dimmed)));
         }
         if is_tool_focused {
      ...
      let group_placeholder = if !self.existing_groups.is_empty() && self.focused_field == group_field {
      ...
      if self.confirm_create_dir.is_some() {
         ...
         let line = Line::from(vec![
            Span::styled("⚠ Path does not exist. Create? ", Style::default().fg(theme.error)),
   ...
   fn render_env_field(
      ...
      if !self.env_list_expanded {
         ...
         let header = Line::from(vec![
            Span::styled("Environment:", label_style),
            Span::styled(" (a)dd (d)el (Enter)edit (Esc)close", Style::default().fg(theme.dimmed)),
         ]);
         lines.push(header);
         ...
         if let Some(ref input) = self.env_editing_input {
            if self.env_adding_new {
               ...
               for (i, entry) in self.extra_env.iter().enumerate() {
                  let prefix = if i == self.env_selected_index { "  > " } else { "    " };
            ...
            if self.extra_env.is_empty() {
               ...
               for (i, entry) in self.extra_env.iter().enumerate() {
                  ...
                  lines.push(Line::from(Span::styled(format!("{}{}", prefix, entry), style)));
               }
            }
         }
      ...
      if show_hook_output {
         ...
         let status_text = if let Some(ref cmd) = self.current_hook {
            ...
            if cmd.len() > max_cmd_len {
               let truncated: String = cmd.chars().take(max_cmd_len.saturating_sub(3)).collect();
               format!("{}...", truncated)
            } else {
               cmd.clone()
            }
