...
fn test_right_arrow_accepts_ghost_text() {
   ...
   assert_eq!(dialog.path.value(), format!("{}/project-alpha/", tmp.path().display()));
}
...
fn test_ghost_recomputes_after_accepting() {
   ...
   assert_eq!(dialog.path.value(), format!("{}/alpha/", tmp.path().display()));
...
fn test_invalid_path_flash_expires_after_tick() {
   let mut dialog = single_tool_dialog();
   dialog.focused_field = 1; // path
   dialog.path_invalid_flash_until = Some(std::time::Instant::now() - std::time::Duration::from_millis(1));
   assert!(dialog.tick());
...
fn test_sandbox_image_input_in_config_mode() {
   ...
   let expected = format!("{}abc", containers::get_container_runtime().effective_default_image());
...
fn test_profile_override_sets_default_tool() {
   ...
   let dialog = NewSessionDialog::new_with_config(vec!["claude", "opencode"], "/tmp/project".to_string(), resolved);
...
fn test_enter_with_existing_path_submits_directly() {
   ...
   let mut dialog = NewSessionDialog::new_with_tools(vec!["claude"], tmp.path().to_string_lossy().to_string());
   let result = dialog.handle_key(key(KeyCode::Enter));
...
fn test_confirm_y_creates_dir_and_submits() {
   ...
   let mut dialog = NewSessionDialog::new_with_tools(vec!["claude"], new_path.to_string_lossy().to_string());
   dialog.confirm_create_dir = Some(false);
...
fn test_confirm_create_failure_shows_error() {
   let mut dialog = NewSessionDialog::new_with_tools(vec!["claude"], "/proc/aoe_test_cannot_create".to_string());
   dialog.confirm_create_dir = Some(true);
   let result = dialog.handle_key(key(KeyCode::Char('y')));
   assert!(matches!(result, DialogResult::Continue));
...
fn test_profile_cycling() {
   let mut dialog = single_tool_dialog();
   dialog.available_profiles = vec!["default".to_string(), "work".to_string(), "personal".to_string()];
