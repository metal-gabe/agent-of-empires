...
use crate::tui::components::{DirPicker, DirPickerResult, GroupGhostCompletion, ListPicker, ListPickerResult};
...
pub(super) const FIELD_HELP: &[FieldHelp] = &[
   ...
   FieldHelp {
      name: "YOLO Mode",
      description: "Skip permission prompts for autonomous operation (--dangerously-skip-permissions)",
   },
   FieldHelp {
   ...
   FieldHelp {
      name: "New Branch",
      description: "Checked: create new branch. Unchecked: use existing (creates worktree if needed)",
   },
   FieldHelp {
      name: "Sandbox",
...
impl NewSessionDialog {
   pub fn new(
      ...
      let profile_index = available_profiles.iter().position(|p| p == profile).unwrap_or(0);
      ...
      Self {
         ...
         sandbox_enabled,
         sandbox_image: Input::new(containers::get_container_runtime().effective_default_image()),
         docker_available,
         yolo_mode,
   ...
   pub fn set_loading(
      ...
      if loading {
         ...
         if self.sandbox_enabled {
            ...
            self.needs_image_pull = !containers::get_container_runtime().image_exists_locally(image);
         }
      }
   }
   ...
   pub(super) fn new_with_config(
      ...
      let tool_index = if let Some(ref default_tool) = config.session.default_tool {
         tools.iter().position(|&t| t == default_tool.as_str()).unwrap_or(0)
      } else {
         0
      };
   ...
   pub fn handle_key(
      ...
      let new_branch_field = if has_worktree { worktree_field + 1 } else { usize::MAX };
      ...
      match key.code {
         ...
         KeyCode::Left | KeyCode::Right | KeyCode::Char(' ') if self.focused_field == profile_field => {
         ...
         KeyCode::Left | KeyCode::Right | KeyCode::Char(' ') if self.focused_field == new_branch_field => {
            self.create_new_branch = !self.create_new_branch;
            DialogResult::Continue
         },
         KeyCode::Left | KeyCode::Right | KeyCode::Char(' ') if self.focused_field == sandbox_field => {
         ...
         KeyCode::Left | KeyCode::Right | KeyCode::Char(' ') if self.focused_field == yolo_mode_field => {
      ...
      match key.code {
         ...
         _ => {
            ...
            if self.sandbox_focused_field == SANDBOX_IMAGE {
               self.sandbox_image.handle_event(&crossterm::event::Event::Key(key));
            }
            DialogResult::Continue
         },
      }
      ...
      match key.code {
         ...
         _ => {
            match self.tool_config_focused_field {
               TOOL_CMD => {
                  self.command_override.handle_event(&crossterm::event::Event::Key(key));
               },
               TOOL_ARGS => {
                  self.extra_args.handle_event(&crossterm::event::Event::Key(key));
               },
               _ => {},
            }
            DialogResult::Continue
   ...
   fn handle_env_list_key(
      &mut self,
      key: KeyEvent,
   ) -> DialogResult<NewSessionData> {
      let validate = |value: &str, list: &[String]| !value.is_empty() && !list.contains(&value.to_string());
   ...
   fn reload_tool_config(&mut self) {
      ...
      let tool = self
         ...
         .copied()
         .unwrap_or("claude");
      self.extra_args = Input::new(config.session.agent_extra_args.get(tool).cloned().unwrap_or_default());
      self.command_override = Input::new(
         config
            .session
            .agent_command_override
   ...
   fn handle_confirm_create_dir_key(
      &mut self,
      key: KeyEvent,
   ) -> DialogResult<NewSessionData> {
