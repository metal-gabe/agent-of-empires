...
impl ProfilePickerDialog {
   pub fn new(
      ...
      let selected = profiles.iter().position(|p| p.name == active_profile).unwrap_or(0);
      Self {
         mode: Mode::List,
         profiles,
         selected,
   ...
   pub fn handle_key(
      &mut self,
      key: KeyEvent,
   ) -> DialogResult<ProfilePickerAction> {
      match self.mode {
   ...
   fn handle_create_key(
      ...
      match key.code {
         ...
         _ => {
            self.name_input.handle_event(&crossterm::event::Event::Key(key));
            self.error = None;
            DialogResult::Continue
         },
      }
   ...
   fn render_list(
         ...
         .enumerate()
      {
         ...
         if profile.is_active {
            spans.push(Span::styled("  (active)", Style::default().fg(theme.running)));
         } else {
      ...
      // Hint line
      let mut hint_spans = vec![Span::styled("n", Style::default().fg(theme.hint)), Span::raw(" new  ")];
      if self.can_delete_selected() {
         hint_spans.extend([
