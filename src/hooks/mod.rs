...
fn build_aoe_hooks() -> Value {
   ...
   let events: &[(&str, &str, Option<&str>)] = &[
      ...
      ("Stop", "idle", None),
      ("Notification", "waiting", Some("permission_prompt|elicitation_dialog")),
   ];
...
pub fn uninstall_hooks(settings_path: &Path) -> Result<bool> {
   ...
   for event_name in event_names {
      if let Some(matchers) = hooks_obj.get_mut(&event_name).and_then(|v| v.as_array_mut()) {
      ...
      for agent in crate::agents::AGENTS {
         if let Some(hook_cfg) = &agent.hook_config {
            ...
            match uninstall_hooks(&settings_path) {
               ...
               Ok(false) => {},
               Err(e) => {
                  tracing::warn!("Failed to remove hooks from {}: {}", settings_path.display(), e);
               },
            }
         }
      }
...
mod tests {
   ...
   fn test_install_hooks_creates_new_file() {
      ...
      let content: Value = serde_json::from_str(&std::fs::read_to_string(&settings_path).unwrap()).unwrap();
   ...
   fn test_hook_command_exits_cleanly_without_aoe_instance_id() {
      ...
      let output = std::process::Command::new("sh")
         .arg("-c")
         .arg(cmd.strip_prefix("sh -c '").unwrap().strip_suffix('\'').unwrap())
   ...
   fn test_uninstall_hooks_removes_aoe_entries() {
      ...
      let content: Value = serde_json::from_str(&std::fs::read_to_string(&settings_path).unwrap()).unwrap();
      assert!(!content.get("hooks").unwrap().as_object().unwrap().is_empty());
