...
fn find_local_image() -> Option<String> {
   let candidates = ["ghcr.io/njbrake/aoe-sandbox:latest", "alpine", "busybox", "ubuntu"];
...
fn extract_volume_to_temp(
   ...
   if !output.status.success() {
      ...
      anyhow::bail!("Failed to extract volume {}: {}", volume_name, stderr.trim());
   }
...
pub fn run() -> Result<()> {
   ...
   for migration in VOLUME_MIGRATIONS {
      ...
      match merge_into(&tmp, &sandbox_dir) {
         ...
         Err(e) => {
            tracing::warn!("Failed to merge files from volume {}: {}", migration.volume_name, e);
         },
      }
