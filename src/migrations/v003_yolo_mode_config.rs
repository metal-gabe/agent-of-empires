...
fn migrate_config_file(path: &PathBuf) -> Result<()> {
   ...
   let Some(yolo_enabled) = yolo_value else {
      debug!("No [sandbox] yolo_mode_default in {}, skipping", path.display());
      return Ok(());
   };
   ...
   if yolo_enabled {
      ...
      session.entry("yolo_mode_default").or_insert(toml::Value::Boolean(true));
   }
...
"#;
      ...
      assert!(result["sandbox"].as_table().unwrap().get("yolo_mode_default").is_none());

      // Other sandbox settings should be preserved
      assert_eq!(result["sandbox"]["enabled_by_default"].as_bool(), Some(false));
   }
...
"#;
      ...
      assert!(result["session"].as_table().unwrap().get("yolo_mode_default").is_none());
   }
...
"#;
      ...
      assert_eq!(result["session"]["yolo_mode_default"].as_bool(), Some(false));
   }
}
