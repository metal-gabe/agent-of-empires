...
const GITHUB_API_LATEST: &str = "https://api.github.com/repos/njbrake/agent-of-empires/releases/latest";
const GITHUB_API_RELEASES: &str = "https://api.github.com/repos/njbrake/agent-of-empires/releases?per_page=20";
...
pub async fn check_for_update(
   ...
   let latest_version = releases.first().map(|r| r.version.clone()).unwrap_or_default();
...
fn filter_releases(
   ...
   match from_version {
      Some(from) => releases.into_iter().take_while(|r| r.version != from).collect(),
      None => releases,
   }
}
...
fn is_newer_version(
   ...
   let parse_version = |v: &str| -> Vec<u32> { v.split('.').filter_map(|s| s.parse().ok()).collect() };
...
mod tests {
   ...
   fn test_cache_should_invalidate_when_current_newer_than_cached() {
      ...
      assert!(!same_version, "same version should not trigger invalidation");
   ...
   fn test_filter_releases_returns_all_when_no_filter() {
      let releases = vec![make_release("0.5.0"), make_release("0.4.3"), make_release("0.4.2")];
