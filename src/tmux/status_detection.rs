...
pub fn detect_opencode_status(raw_content: &str) -> Status {
   ...
   let permission_prompts = ["(y/n)", "[y/n]", "continue?", "proceed?", "approve", "allow"];
   ...
   for line in &lines {
      ...
      if trimmed.starts_with("❯") && trimmed.len() > 2 {
         ...
         if after_cursor.starts_with("1.") || after_cursor.starts_with("2.") || after_cursor.starts_with("3.") {
            return Status::Waiting;
         }
      }
   }
   if lines
      .iter()
      .any(|line| line.contains("❯") && (line.contains(" 1.") || line.contains(" 2.") || line.contains(" 3.")))
   {
      return Status::Waiting;
   }
   ...
   let has_completion = completion_indicators.iter().any(|ind| last_lines_lower.contains(ind));
   if has_completion {
      for line in non_empty_lines.iter().rev().take(10) {
...
mod tests {
   ...
   fn test_detect_vibe_status_waiting() {
      ...
      assert_eq!(detect_vibe_status("⚠ bash command\nExecute this?"), Status::Waiting);
      // Approval options
      assert_eq!(
         detect_vibe_status("› Yes\n  Yes and always allow bash for this session\n  No and tell the agent"),
         Status::Waiting
      );
   }
   ...
   fn test_detect_codex_status_running() {
      ...
      assert_eq!(detect_codex_status("thinking about your request"), Status::Running);
   ...
   fn test_detect_codex_status_waiting() {
      assert_eq!(detect_codex_status("run this command? (y/n)"), Status::Waiting);
      assert_eq!(detect_codex_status("approve changes?"), Status::Waiting);
      assert_eq!(detect_codex_status("execute this action? [y/n]"), Status::Waiting);
   ...
   fn test_detect_gemini_status_waiting() {
      assert_eq!(detect_gemini_status("run this command? (y/n)"), Status::Waiting);
      assert_eq!(detect_gemini_status("approve changes?"), Status::Waiting);
      assert_eq!(detect_gemini_status("execute this action? [y/n]"), Status::Waiting);
   ...
   fn test_detect_pi_status_waiting() {
      ...
      assert_eq!(detect_pi_status("reading config.toml\nDone.\n>"), Status::Waiting);
   }
