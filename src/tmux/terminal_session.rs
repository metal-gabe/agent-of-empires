...
use super::{refresh_session_cache, session_exists_from_cache, CONTAINER_TERMINAL_PREFIX, TERMINAL_PREFIX};
...
impl TerminalSession {
   ...
   pub fn kill(&self) -> Result<()> {
      ...
      let output = Command::new("tmux").args(["kill-session", "-t", &self.name]).output()?;
   ...
   pub fn capture_pane(
      ...
      let output = Command::new("tmux")
         .args(["capture-pane", "-t", &self.name, "-p", "-S", &format!("-{}", lines)])
         .output()?;
...
impl ContainerTerminalSession {
   ...
   pub fn generate_name(
      ...
      let safe_title = sanitize_session_name(title);
      format!("{}{}_{}", CONTAINER_TERMINAL_PREFIX, safe_title, truncate_id(id, 8))
   }
...
mod tests {
   ...
   fn test_build_terminal_create_args_without_size() {
      ...
      assert_eq!(
         args,
         vec!["new-session", "-d", "-s", "test_terminal", "-c", "/tmp/work"]
      );
