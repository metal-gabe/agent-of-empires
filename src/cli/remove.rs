...
pub async fn run(
   ...
   for inst in instances {
      if inst.id == args.identifier || inst.id.starts_with(&args.identifier) || inst.title == args.identifier {
         found = true;
         ...
         let user_confirmed = if will_cleanup_worktree {
            ...
            println!("\nThis will delete:");
            println!("  - Worktree: {} (branch: {})", inst.project_path, wt_info.branch);
            print!("\nProceed? (Y/n): ");
            io::stdout().flush()?;
            ...
            if user_confirmed {
               ...
               let sandbox_cleaned = if inst.is_sandboxed() {
                  ...
                  if container.exists().unwrap_or(false) {
                     ...
                     container.exec(&["find", ".", "-mindepth", "1", "-delete"]).is_ok()
                  } else {
                     false
                  }
               } else {
         ...
         if let Ok(tmux_session) = crate::tmux::Session::new(&inst.id, &inst.title) {
            if tmux_session.exists() {
               if let Err(e) = tmux_session.kill() {
                  ...
                  eprintln!("Session removed from Agent of Empires but may still be running in tmux");
               }
            }
         }
