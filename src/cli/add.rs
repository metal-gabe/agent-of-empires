...
pub async fn run(
   ...
   let final_title = if let Some(title) = &args.title {
      ...
      if is_duplicate_session(&instances, trimmed_title, path.to_str().unwrap_or("")) {
         println!("Session already exists with same title and path: {}", trimmed_title);
         return Ok(());
      }
   ...
   if let Err(e) = hook_result {
      ...
      if let Some(ref wt_info) = instance.worktree_info {
         if wt_info.managed_by_aoe {
            if let Ok(git_wt) = crate::git::GitWorktree::new(std::path::PathBuf::from(&wt_info.main_repo_path)) {
   ...
   if args.launch {
      ...
      println!();
      println!("Next steps:");
      println!("  agent-of-empires session start {}   # Start the session", final_title);
