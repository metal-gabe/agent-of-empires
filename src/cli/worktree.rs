...
async fn show_info(
   ...
   if let Some(wt_info) = &session.worktree_info {
      ...
      println!(
         "  Cleanup on delete: {}",
         if wt_info.cleanup_on_delete { "Yes" } else { "No" }
      );
      println!("  Created at:    {}", wt_info.created_at.format("%Y-%m-%d %H:%M:%S"));
      ...
      bail!("Session '{}' is not associated with a worktree", session.title);
   }
...
async fn cleanup_orphaned(
   ...
   if GitWorktree::is_git_repo(&current_dir) {
      ...
      for wt in worktrees {
         ...
         let is_tracked = instances.iter().any(|inst| inst.project_path == wt_path_str);
   ...
   println!("  - Delete {} worktree directories", orphaned_worktrees.len());
   print!("(y/N): ");
   io::stdout().flush()?;
