...
"#,
   ...
   assert_eq!(config.session.unwrap().default_tool, Some("claude".to_string()));
}
...
fn test_trust_untrust_cycle() {
   ...
   let is_trusted = agent_of_empires::session::repo_config::is_repo_trusted(project_path, hooks_hash).unwrap();
   assert!(!is_trusted);
   ...
   let is_trusted = agent_of_empires::session::repo_config::is_repo_trusted(project_path, "different_hash").unwrap();
   assert!(!is_trusted);
   ...
   assert!(!is_trusted);
   // New hash is trusted
   let is_trusted = agent_of_empires::session::repo_config::is_repo_trusted(project_path, "new_hash").unwrap();
   assert!(is_trusted);
}
...
fn test_hook_execution_failure() {
   let tmp = TempDir::new().unwrap();
   let result = agent_of_empires::session::repo_config::execute_hooks(&["exit 1".to_string()], tmp.path());
...
fn test_changed_hooks_invalidate_trust() {
   ...
   assert_ne!(hash_v1, hash_v2, "different hooks should produce different hashes");
}
