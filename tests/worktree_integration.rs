...
fn test_add_session_with_worktree_flag() {
   ...
   git_wt.create_worktree("test-feature", &wt_path, false).unwrap();
...
fn test_session_has_worktree_info_after_creation() {
   ...
   let info = instance.worktree_info.as_ref().unwrap();
   assert_eq!(info.branch, "test-feature");
   assert_eq!(info.main_repo_path, repo_dir.path().to_string_lossy().to_string());
   assert!(info.managed_by_aoe);
...
fn test_manual_worktree_detection() {
   ...
   let added_wt = worktrees
      .iter()
      .find(|w| w.branch.as_ref().map(|b| b == "test-feature").unwrap_or(false));
...
fn test_create_new_branch_with_b_flag() {
   ...
   let branch_exists_before = repo.find_branch("brand-new-branch", git2::BranchType::Local).is_ok();
   assert!(!branch_exists_before);

   git_wt.create_worktree("brand-new-branch", &wt_path, true).unwrap();

   assert!(wt_path.exists());

   let branch_exists_after = repo.find_branch("brand-new-branch", git2::BranchType::Local).is_ok();
