...
use agent_of_empires::session::{
   load_profile_config, merge_configs, save_config, save_profile_config, Config, ProfileConfig, SandboxConfigOverride,
...
fn test_merge_overrides_global() -> Result<()> {
   ...
   assert!(!merged.sandbox.auto_cleanup, "Profile override should take precedence");
...
fn test_merge_inherits_unset_fields() -> Result<()> {
   ...
   assert!(merged.worktree.enabled, "worktree.enabled should inherit from global");
