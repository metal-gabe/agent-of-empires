...
fn convert_cast_to_gif(cast_path: &Path) {
   if !agg_available() {
      eprintln!("agg not found -- skipping GIF conversion for {}", cast_path.display());
      return;
   }
...
impl TuiTestHarness {
   ...
   pub fn new(test_name: &str) -> Self {
      ...
      #[cfg(unix)]
      {
         ...
         std::fs::set_permissions(&claude_stub, std::fs::Permissions::from_mode(0o755)).expect("chmod claude stub");
      }
...
"#,
      ...
      std::fs::create_dir_all(config_dir.join("profiles").join("default")).expect("create default profile dir");
   ...
   pub fn assert_screen_not_contains(
      &self,
      text: &str,
   ) {
      let screen = self.capture_screen();
