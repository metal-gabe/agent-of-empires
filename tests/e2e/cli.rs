...
fn read_sessions_json(h: &TuiTestHarness) -> serde_json::Value {
   let sessions_path = if cfg!(target_os = "linux") {
      ...
      h.home_path().join(".agent-of-empires/profiles/default/sessions.json")
   };
...
fn test_cli_add_invalid_path() {
   ...
   assert!(!output.status.success(), "aoe add should fail for nonexistent path");
...
"#,
   ...
   let add_output = h.run_cli(&["add", project.to_str().unwrap(), "-t", "CmdOverride", "--cmd", "claude"]);
   assert!(
      add_output.status.success(),
      "aoe add failed: {}",
      String::from_utf8_lossy(&add_output.stderr)
